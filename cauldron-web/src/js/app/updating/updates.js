...
function prepareStepBody(step) {
  ...
  stepBody.find('[data-src]').each((index, e) => {
    ...
    element.attr(
      'src',
      `${cauldron.DATA_DIRECTORY}/${src}?nocache=${cauldron.getNoCacheString()}`
    );
...
function onReadyToProcess(updates) {
  ...
  updates.forEach((update) => {
    ...
    let stepDom = prepareStepBody(update.step);
...
function processStepUpdates(updates, stepToSelect) {
  ...
  processingPromise = processingPromise
    .then(() => loader.loadStepIncludes(steps))
